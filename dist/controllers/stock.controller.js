...
const SellNormal = asynchandler(async (req, res) => {
    const { stock_name, quantity, curr_price } = req.body;
    if (!stock_name || !quantity || !curr_price) {
        throw new ApiError(400, "All fields Required");
    }
    ...
    const holding1 = await db
        .select()
        .from(Holdings)
        .where(and(eq(Holdings.user_id, user.id), eq(Holdings.stock_name, stock_name)));
...
export { BuyNormal, SellNormal, addOption, getHoldings, getTransactions, getWatchlists, addToWatchList, deleteFromWatchList, updateWatchList, };
